...
class dataset_Hypergraph(InMemoryDataset):
    def __init__(self, root='../data/pyg_data/hypergraph_dataset/', name=None,
        ...
        existing_dataset = ['mimic3', 'cradle', 'promote', 'eunomia', 'synpuf1k']
    ...
    def download(self):
        for name in self.raw_file_names:
            ...
            if not osp.isfile(p2f):
                ...
                elif self.name in ['eunomia']:
                    tmp_data = load_dataset(
                        path=self.p2raw,
                        dataset=self.name,  # 'eunomia'
                        node_feature_path="../data/raw_data/eunomia/node-embeddings-eunomia",
                        num_node=self.num_nodes
                    )
                    
                elif self.name in ['synpuf1k']:
                    tmp_data = load_dataset(
                        path=self.p2raw,
                        dataset=self.name, 
                        node_feature_path="../data/raw_data/synpuf1k/node-embeddings-synpuf1k",
                        num_node=self.num_nodes
                    )
