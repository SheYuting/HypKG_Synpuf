...
def parse_method(args, data):
    model = None
    if args.dname in ['mimic3', 'cradle', 'promote', 'eunomia', 'synpuf1k']:
...
def eval_cradle(y_true, y_pred, epoch, method, dname, args, mode='dev', threshold=0.5):
    ...
    # If y_true is all 0 or all 1, ROC/AUPR are undefined.
    if np.unique(y_true).size < 2:
        # You can choose 0.0 or np.nan; np.nan makes it obvious in logs.
        roc_auc = float("nan")
        aupr = float("nan")
        return accuracy, roc_auc, aupr, f1_macro
...
if __name__ == '__main__':
    ...
    with torch.autograd.set_detect_anomaly(True):
        for epoch in trange(args.epochs):
            ...
            elif dname in ['cradle', 'promote', 'eunomia', 'synpuf1k']:
            ...
            if args.vanilla: vanilla = "_vanilla"
            if dname == 'mimic3':
                fname_dev = f'outputs/mimic3_dev_{args.method}{vanilla}_{args.rand_seed}.txt'
                fname_test = f'outputs/mimic3_test_{args.method}{vanilla}_{args.rand_seed}.txt'
            elif dname == 'cradle':
                fname_dev = f'outputs/cradle_dev_{args.method}{vanilla}_{args.rand_seed}.txt'
                fname_test = f'outputs/cradle_test_{args.method}{vanilla}_{args.rand_seed}.txt'
            elif dname == 'promote':
                fname_dev = f'outputs/promote_dev_{args.method}{vanilla}_{args.rand_seed}.txt'
                fname_test = f'outputs/promote_test_{args.method}{vanilla}_{args.rand_seed}.txt'
            elif dname == 'eunomia':
                fname_dev = f'outputs/eunomia_dev_{args.method}{vanilla}_{args.rand_seed}.txt'
                fname_test = f'outputs/eunomia_test_{args.method}{vanilla}_{args.rand_seed}.txt'
            elif dname == 'synpuf1k':
                fname_dev = f'outputs/synpuf1k_dev_{args.method}{vanilla}_{args.rand_seed}.txt'
                fname_test = f'outputs/synpuf1k_test_{args.method}{vanilla}_{args.rand_seed}.txt'
