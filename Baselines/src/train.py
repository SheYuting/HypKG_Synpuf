...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_labels', default=25, type=int)  # 1 for cradle and promote, 25 for mimic
    ...
    # avgnode or multihot
    parser.add_argument('--num_labeled_data', default=12353, type=str)
    ...
    if args.logistic_regression:
        ...
        if args.dname in ['mimic3']:
            model = OneVsRestClassifier(LogisticRegression(max_iter=1000, random_state=args.rand_seed, C=1))
            model.fit(features[train_idx], labels[train_idx])
            val_pred = model.predict(features[valid_idx])
    ...
    if args.svm:    # it takes a really long time to run svm
        print('-'*10 + 'SVM' + '-'*10)
        if args.dname in ['cradle', 'promote']:
            model = SVC(kernel='rbf', C=10, gamma='scale', coef0=0, random_state=args.rand_seed, probability=True)
            model.fit(features[train_idx], labels[train_idx])
            val_pred = model.predict(features[valid_idx])
            val_prob = model.predict_proba(features[valid_idx])[:, 1]
            test_pred = model.predict(features[test_idx])
    ...
    if args.mlp:
        print('-'*10 + 'MLP' + '-'*10)
        if args.dname in ['cradle', 'promote']:
            model = MLPClassifier(hidden_layer_sizes=(32, 16), activation='relu',alpha=0.0001, batch_size='auto', learning_rate='constant', learning_rate_init=1e-3, validation_fraction=0, max_iter=1000, random_state=args.rand_seed)
            model.fit(features[train_idx], labels[train_idx])
            val_pred = model.predict(features[valid_idx])
            val_prob = model.predict_proba(features[valid_idx])[:, 1]
            test_pred = model.predict(features[test_idx])
            ...
            eval_cradle(labels[test_idx], test_prob, )
        if args.dname in ['mimic3']:
            model = OneVsRestClassifier(MLPClassifier(hidden_layer_sizes=(64, 64), activation='relu',alpha=0.0001, batch_size='auto', learning_rate='constant', learning_rate_init=1e-3, validation_fraction=0, max_iter=1000, random_state=args.rand_seed))
            model.fit(features[train_idx], labels[train_idx])
            val_pred = model.predict(features[valid_idx])
            val_prob = model.predict_proba(features[valid_idx])
            test_pred = model.predict(features[test_idx])
    ...
    if args.random_forest:
        ...
        if args.dname in ['cradle', 'promote']:
            ...
            eval_cradle(labels[test_idx], test_prob, )
        if args.dname in ['mimic3']:
            model = OneVsRestClassifier(RandomForestClassifier(n_estimators=500, max_depth=10, min_samples_split=2, min_samples_leaf=1, random_state=args.rand_seed))
            model.fit(features[train_idx], labels[train_idx])
            val_pred = model.predict(features[valid_idx])
            val_prob = model.predict_proba(features[valid_idx])
            test_pred = model.predict(features[test_idx])
    ...
    if args.xgboost:
        ...
        if args.dname in ['cradle', 'promote']:
            ...
            eval_cradle(labels[test_idx], test_prob, )
        if args.dname in ['mimic3']:
            model = OneVsRestClassifier(XGBClassifier(n_estimators=50, max_depth=10, max_leaves=10, learning_rate=0.5, min_child_weight=10, random_state=args.rand_seed, ))
            model.fit(features[train_idx], labels[train_idx])
